...
class UltraSuperCalculator:
  def __init__(self,name):
    self.name=name
    self.number_registers=[0 for x in range(22)]
    self.history_registers=[0 for x in range(22,32)]
    self.numbers_index=1
    self.history_index=0
    self.temp_history_index=0
    self.user_display=""
    my_input="MEDO"
    self.update_display(my_input)
    return None
  def update_display(self,to_update):
    self.user_display=to_update
    print(f"Hello {self.user_display}")
  ...
  def get_last_calculation(self):
    ...
    last_value=f"Last result= {int(history_registers[temp_history_index],2)}"
...
helloThing=UltraSuperCalculator('Dawoud')

# Adds 5 and 10 to number registers
helloThing.binary_reader("00000100000000000000000101000000")
helloThing.binary_reader("00000100000000000000001010000000")

# Adds/Subtracts/Multiplies/Divides 5 and 10 from registers
helloThing.binary_reader("00000000001000100000000000100000")
helloThing.binary_reader("00000000001000100000000000100010")
helloThing.binary_reader("00000000001000100000000000011000")
helloThing.binary_reader("00000000001000100000000000011010")

# Gets the last three calculations
helloThing.binary_reader("10000100000000000000000000000000")
helloThing.binary_reader("10000100000000000000000000000000")
helloThing.binary_reader("10000100000000000000000000000000")
